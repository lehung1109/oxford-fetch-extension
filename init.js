...
(function($) {
    var AutoCompleter = function(element, settings) {
        ...
        obj.settings = $.extend({
            url: "/autocomplete",
            // url used for the autocomplete
            resultKey: "results",
            // name of the key in the JSON result
            method: "GET",
            // method to do the AJAX request
            delay: 500,
            // delay in ms between invocation of the autocomplete
            minChars: 1,
            // minimum number of characters to have in order to trigger the autocomplete
            maxQueriesToCache: 5,
            // hide five queries
            autocompleterClass: "autocompleter",
            //class of the autocompleter
            autocompleterResultClass: "autocompleterResult",
            queryCallback: null,
            // query to do the callback
            selectRowCallback: null,
            // callback triggered when a row is selected
            confirmSuggestionCallback: null,
            // callback after a suggestion is confirmed
            createResultRowCallback: null,
            // callback to create a row to add to the autocomplete popup
            footerLink: null,
            // if	!= null, the link will be added to the bottom of the autocomplete list
            footerLinkCallback: null,
            // if != null, the function will be called clicking on the footerLink
            timeout: null
        }, settings || {});
...
window.oldoApi = {
    ...
    _defaultFailureCb: function(response, requestOptions) {
        ...
        if (response.status == 401) {
            ...
            window.location.href = window.oldoApi._baseUrl + "account/login?callbackUrl=" + encodeURIComponent(window.location.href);
        } else {
...
var URLS = {
    "spinner_small": "/external/images/spinner_small.gif?version=2.3.48",
...
function supportAudioHtml5() {
    ...
    try {
        return (!!(audioTag.canPlayType) && ((audioTag.canPlayType("audio/mpeg") != "no" && audioTag.canPlayType("audio/mpeg") != "") || (audioTag.canPlayType("audio/ogg") != "no" && audioTag.canPlayType("audio/ogg") != "")));
    } catch (e) {
        return false;
    }
}
...
(function($) {
    ...
    $.fn.customSelector = function(settings) {
        this.each(function() {
            new CustomSelector(this,settings);
        });
    }
    ;
}
...
function createLetCopy(letters) {
    ...
    for (k = 0; k < lettersA.length; k++) {
        lettersA[k].addEventListener("click", function() {
            ...
            if (letter != "") {
                //do not consider the arrow as a letter
...
$(doc).ready(function() {
    ...
    if ((typeof (contextId) == "undefined" || !contextId) && window.matchMedia("(min-width: 762px)").matches && $(doc).scrollTop() == 0 && !("ontouchstart"in doc.documentElement)) {
        $("#q").focus();
    }

    var pxShow = $(window).height() / 3;
    ...
    $(".toc-toggle").click(function() {
        ...
        if (element.hasClass("data-fold")) {
            ...
            $.cookie("tocUnFold", true, {
                expires: 7,
                path: '/'
            });
        } else {
            element.addClass("data-fold");
            $.cookie("tocUnFold", null, {
                expires: 7,
                path: '/'
            });
        }
        ...
        createResultRowCallback: function(x, y) {
            ...
            for (var result in y) {
                if (dictCodesArr[x].url != null)
                    suggestionListUl.append("<li><a class='suggestionResult' data-value='" + dictCodesArr[x].url + "'>" + y[result].searchText + "</a></li>");
            }
...
function initDictionarySelector() {
    ...
    var userDictionary = (doc.location.href.indexOf('/grammar/') > 0 && doc.location.href.indexOf('/online-grammar/') == -1) ? "practical-english-usage" : $.cookie("dictionary");
...
function createUserVersionCookie(id) {
    ...
    if (version == "UK")
        $.cookie('dictionary', "english", {
            expires: 7,
            path: '/'
        });
    else if (version == "US")
        $.cookie('dictionary', "american_english", {
            expires: 7,
            path: '/'
        });
}
...
function iPadTrigger() {
    if (!navigator.userAgent.match(/(iphone|ipod|ipad|macintosh)/i) || navigator.userAgent.match(/iemobile/i) || navigator.userAgent.match(/chrome/i))
        return;
